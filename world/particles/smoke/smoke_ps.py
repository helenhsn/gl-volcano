...
class ParticleSystem:
    def __init__(self):
        ...
        print(f"shape = {len(self.positions)} && positiosn = {self.positions}")
        ...
        for i in range(self.nb_particles):
            factor = 1.0
            phi = uniform(0.0, pi/10.0)
            theta = uniform(0.0, two_pi)

            speed_slope_x = R * factor * sin(phi) * cos(theta) * uniform(1.0, 5.0)
            speed_slope_y = R * factor * cos(phi) * uniform(1.0, 4.0)
            speed_slope_z = R * factor * sin(phi) * sin(theta) * uniform(1.0, 5.0)
        ...
        # binding the ssbos to the compute shader program && generating buffers vel=np.array(self.velocities, dtype=np.float32)
